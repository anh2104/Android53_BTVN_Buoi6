...
//        }
    ...
    public  boolean updateProduct(int productId, ProductModel productModel){
        if(productId >= 0 && productModel != null){
            SQLiteDatabase db = getWritableDatabase();
            String whereClause = PRODUCT_ID + " =?";
            String[] whereArgs = {productId + ""};
            ContentValues contentValues = new ContentValues();
            contentValues.put(PRODUCT_TITLE,productModel.getTitle());
            contentValues.put(PRODUCT_DES,productModel.getDescription());
            contentValues.put(PRODUCT_PRICE,productModel.getPrice());
            contentValues.put(PRODUCT_DISCOUNT,productModel.getDiscountPercentage());
            contentValues.put(PRODUCT_STOCK,productModel.getStock());
            contentValues.put(PRODUCT_BRAND,productModel.getBrand());
            contentValues.put(PRODUCT_CATEGORY,productModel.getCategory());
            contentValues.put(PRODUCT_THUMBNAIL,productModel.getThumbnail());

            Gson gson = new Gson();
            Type typeToken = new TypeToken<List<String>>(){}.getType();
            String data = gson.toJson(productModel.getImages(),typeToken);
            contentValues.put(PRODUCT_IMAGES ,data);
    ...
    @SuppressLint("Range")
    public List<ProductModel> getProductsByTitle(String title){
        List<ProductModel > listproduct = new ArrayList<>();
        if (title == null || title.isEmpty()) {
            // Xử lý trường hợp title rỗng hoặc rỗng
            return listproduct;
        }
        SQLiteDatabase db = getReadableDatabase();
